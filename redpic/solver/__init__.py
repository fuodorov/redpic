...
class Simulation:
    ...
    def _get_field_accelerator(
        self, acc: Accelerator, type: str, x: np.array, y: np.array, z: np.array
    ...
    def _sum_field_particles(
        self, x: np.array, y: np.array, z: np.array, z_start: float, z_stop: float
    ) -> (np.array, np.array, np.array):
        n = int(len(x))
        Fx = np.zeros(n)
        Fy = np.zeros(n)
            ...
            if z_start <= z[i] <= z_stop:
                for j in prange(n):  # pylint: disable=E1133
                    if z_start <= z[j] <= z_stop and i != j:
                        Fx[i] += (x[i] - x[j]) / ((x[j] - x[i]) ** 2 + (y[j] - y[i]) ** 2 + (z[j] - z[i]) ** 2) ** (
                            3 / 2
                        )
                        Fy[i] += (y[i] - y[j]) / ((x[j] - x[i]) ** 2 + (y[j] - y[i]) ** 2 + (z[j] - z[i]) ** 2) ** (
                            3 / 2
                        )
                        Fz[i] += (z[i] - z[j]) / ((x[j] - x[i]) ** 2 + (y[j] - y[i]) ** 2 + (z[j] - z[i]) ** 2) ** (
                            3 / 2
                        )
        return Fx, Fy, Fz
    ...
    def _get_field_beam(
        ...
        q = Q / beam.n
        if type == "E":
            Ex, Ey, Ez = self._sum_field_particles(x, y, z, acc.z_start, acc.z_stop)
            return (
        ...
        if type == "B":
            Bx, By, Bz = self._sum_field_particles(x, y, z, acc.z_start, acc.z_stop)
            return (
                const.km / (4 * np.pi) * q * Bx,
                -const.km / (4 * np.pi) * q * By,
                0 * const.km / (4 * np.pi) * q * Bz,
            )
    ...
    def track(self, *, n_files: int = 20) -> None:
        ...
        for t in np.arange(0, t_max, 2 * dt):
            ...
            Ex, Ey, Ez = self._get_field_accelerator(self.acc, "E", Y[0] * dz, Y[1] * dz, Y[2] * dz)
            Ex, Ey, Ez = Ex / E0, Ey / E0, Ez / E0

            # get electric field from beam
            if self.beam.total_charge:
                ex, ey, ez = self._get_field_beam(self.beam, self.acc, "E", Y[0] * dz, Y[1] * dz, Y[2] * dz)
                ex, ey, ez = ex / E0, ey / E0, ez / E0
            ...
            Bx, By, Bz = self._get_field_accelerator(self.acc, "B", Y[0] * dz, Y[1] * dz, Y[2] * dz)
            Bx, By, Bz = Bx / B0 / gamma, By / B0 / gamma, Bz / B0 / gamma

            # get magnetic field from beam
            if self.beam.total_charge:
                bx, by, bz = self._get_field_beam(self.beam, self.acc, "B", Y[0] * dz, Y[1] * dz, Y[2] * dz)
